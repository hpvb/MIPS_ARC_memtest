...
static const char *numtostr(unsigned long num, unsigned int base, int width)
{
	static char str[33];
	static char digits[] = {
		'0', '1', '2', '3', '4', '5', '6', '7',
		...
		while (*format != '\0') {
			...
			if (str == NULL) {
				...
				case 'p':
					...
					str = numtostr(ulong, 16, sizeof(unsigned long)*2);
					if (fputs(str, stream) == EOF)
						return EOF;
					count += strlen(str);
					break;
